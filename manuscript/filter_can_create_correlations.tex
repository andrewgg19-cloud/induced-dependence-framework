...
\begin{proposition}[Induced Correlation Threshold]
Let $D_{N}$ be a natural distribution with zero covariance between features $X_{1}$ and $X_{2}$. If an informative filter $F$ satisfies $I(\{X_1, X_2\}; F) > 0$, the observable distribution $D_{F} \sim P(R \mid F=1)$ will exhibit an emergent covariance $\Sigma_{1,2} \neq 0$ bounded strictly by the observational deformation:
\begin{equation}
|\Sigma_{1,2}| \leq \zeta \cdot \Delta_{obs}
\end{equation}
where $\zeta \in \mathbb{R}^+$ is a scaling coefficient proportional to the truncation density, and $\Delta_{obs} = d(D_N, D_F)$. If the filter completely restricts the support of $D_N$, $\Delta_{obs} \to \infty$, rendering any emergent correlation a deterministic artifact of the selection boundary rather than an intrinsic systemic property.
\end{proposition}
...
\section{Minimal Computational Diagnostic}
To make the framework reproducible, a minimal computational diagnostic is included in the repository. The diagnostic models a Natural State $D_N$ as a Gaussian distribution and applies a rigid lower truncation threshold to generate a Filtered Observable State $D_F$. The filter removes part of the support of the natural distribution and therefore produces measurable observational deformation.

For the baseline parameters:
\begin{equation}
\mu_N = 0,\quad \sigma_N = 1,\quad F: R > 0.5
\end{equation}
the diagnostic reports:
\begin{equation}
\Delta_{obs} = D_{KL}(D_F \parallel D_N) = 1.1759 \text{ nats}
\end{equation}
and:
\begin{equation}
D_{KL}(D_N \parallel D_F) = \infty
\end{equation}
because the filtered distribution assigns zero probability to regions that remain possible under the natural distribution.

The same run estimates:
\begin{equation}
H(D_F) = 0.5283 \text{ nats}
\end{equation}
and a variance destruction of:
\begin{equation}
Var(D_N) - Var(D_F) = 0.7315
\end{equation}

This diagnostic clarifies an important asymmetry: when a filter truncates support, the direction of KL divergence matters. $D_{KL}(D_N \parallel D_F)$ identifies the impossibility introduced by the filter, while $D_{KL}(D_F \parallel D_N)$ provides a finite reproducible measure of the deformation of the observable state relative to the natural state.
